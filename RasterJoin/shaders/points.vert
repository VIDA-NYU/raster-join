...
layout(location=1) in  float attributes[6];
...
float floatVal(int aid) {
    float ret = 0;
    int typ = int(type[aid].x);
    switch(typ) {
    case Uint:
        ret = float(floatBitsToUint(attributes[aid-1]));
        break;

    case Int:
        ret = float(floatBitsToInt(attributes[aid-1]));
        break;

    case Float:
        ret = attributes[aid-1];
        break;
