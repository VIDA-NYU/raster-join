...
public:
	...
	//char ** getAttributesAsBinary() {
		//char** binArray = new char*[getNbAttributes()];
		//size_t byte_size = 0; //DEBUG
		
		//for (uint64_t i = 0 ; i < getNbAttributes() ; i++) {
			//binArray[i] = getColAsBinary(i);
			//byte_size += sizeof(binArray[i]); //DEBUG
		//}
		
		//assert(byte_size == getRecordSize()); //DEBUGs
		//return binArray;
	//}
	
    STdims getIndexDimensions() {
        STdims indexDims;
