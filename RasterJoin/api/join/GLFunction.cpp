...
void GLFunction::setMaxCellSize(double size) {
    cellSize = (size / std::sqrt(2));
}

NPArray GLFunction::execute() {
#ifdef PROFILE_GL
...
#endif
    QVector<int> agg = this->executeFunction();
    return createResult(agg);
}

NPArray GLFunction::createResult(QVector<int> &agg) {
    NPArray res;
    res.size = polySize;
    res.type = NP_Float;
    ...
    int attrid;
    this->handler->dataHandler->getAggregation(aggrType, attrid);

    Value *resVals = new Value[polySize];
    for(uint32_t i = 0;i < polySize;i ++) {
        switch(aggrType) {
        case Aggregation::Avg:
        {
            float num = float(result[polySize + i]) * 1000.f + result[2 * polySize + i] / 10.f;
            float den = result[i];
            resVals[i].fval = (result[i] == 0)? 0: num / den;
            break;
        }

        case Aggregation::Sum:
            resVals[i].fval = float(result[polySize + i]) * 1000.f + result[2 * polySize + i] / 10.f;
            break;

        default:
            resVals[i].fval = float(result[i]);
            break;
        }
    }
    res.data = (void *) resVals;
...
void GLFunction::setupPoints() {
    ...
    size_t location_attribute = res->locAttribute;
    ...
    DataColumn binAttribResult = res->columns[location_attribute];
...
//    qDebug() << "Number of records in the result" << result_size;
    ...
    if(other_attributes.size() > 0) {
        ...
        for (set<size_t>::iterator it=other_attributes.begin(); it!=other_attributes.end(); ++it) {
            binAttribResult = res->columns[*it];
            type = res->columns[*it].type;
