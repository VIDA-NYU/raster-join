...
ForEach($polySize in $polySizes) {
	...
	$arguments = "--nIter", 6, "--joinType", "raster", "--accuracy", 10, "--backendIndexName", "$index", "--locAttrib", 1, "--polygonList", "$polyList", "--polygonDataset", "$polySize", "--startTime", 1230768000, "--endTime", 1341128000, "--outputTime", "$opFile", "--gpuMem", $GpuMem
